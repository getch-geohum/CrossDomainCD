...
def OT_loss(xs, xt):
    '''
    Computes optimum transpor loss, a loss that looks on the transport plan for mapping the source and target domains, here different change detection datasets
    xs: source chnage features or features from labeled change dataset
    xt: target change features or features from labeld chnage dataset
    ...
    xs = xs.reshape(x_size[0], -1)
    xt = xt.reshape(y_size[0], -1)

    M = torch.Tensor(cdist(xs.detach().cpu().numpy(),xt.detach().cpu().numpy(), metric='sqeuclidean'))
    M = M.cuda()
    M = M/M.max()  # normalized cost matrix


    a, b = ot.unif(xs.size()[0]), ot.unif(xt.size()[0])
    gamma_ot = ot.emd(a, b, M.detach().cpu().numpy())    # Earth movers distance 
