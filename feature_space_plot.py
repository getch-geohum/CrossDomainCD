...
#                 length.append(imgs.shape[0])
    ...
    def plot_joint_single_space(self):
        ...
        camps_ = {'EGY-BCD': 'EGY-BCD',
                  'Nguenygiel':'Nguenygiel',
                  'Nduta': 'Nduta',
                  'Minawao': 'Minawao',
                  'LEVIR-CD': 'LEVIR-CD',
                  'Kuletirkidi': 'Kuletirkidi',
                  'WHU-CD':'WHU-CD'}
        index_site_name = self.out_root + '/sit_ind_sit_name.json'
        with open(index_site_name) as jopen_file:
            sites = ast.literal_eval(json.load(jopen_file))
...
def argumentParser():
    ...
    parser.add_argument('--data_root', help='data folder, can be either with single task or multi task', type=str, required=False, default='/home/getch/meta/SemiCD/deep_feat_noabs')
    parser.add_argument('--save_dir', help = 'main root to save the test result', type = str, required=False, default='/home/getch/meta/SemiCD/deep_feat_plots_noabs')
